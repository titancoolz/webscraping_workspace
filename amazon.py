...
class AmazonProductScraper:
    ...
    def get_category_url(self,input_search_tearm):

        # self.category_name = input("\n>> Enter the product/category to be searched: ")
        self.category_name = str(input_search_tearm)
    ...
    @staticmethod
    def extract_product_information(page_results,subgoals):
        temp_record = []
        for i in range(len(page_results)):
            ...
            product_information = (description, Original_product_price[1:], product_price[1:], product_review, review_number, category_url, Sponsored,subgoals)
    ...
    def navigate_to_other_pages(self, category_url,number_of_pages_to_search,records,subgoals):
        ...
        for i in range(2, 2+number_of_pages_to_search):
            # Goes to next page
            ...
            temp_record = self.extract_product_information(page_results,subgoals)
    ...
    def product_information_spreadsheet(self, records):
        ...
        for _ in records:
            ...
            writer = csv.writer(f)
            writer.writerow(['Description', 'original Price', 'discounted Price', 'Rating', 'Review Count', 'Product URL', 'Ad or not?','subgoal' ])
...
if __name__ == "__main__":
    ...
    for i, j in df.iterrows():
        ...
        for x in my_list:
            # print(x)
            input_search_tearm=x

            my_amazon_bot = AmazonProductScraper()

            my_amazon_bot.open_browser()

            category_details = my_amazon_bot.get_category_url(input_search_tearm)

            my_amazon_bot.extract_product_information(my_amazon_bot.extract_webpage_information())

            navigation = my_amazon_bot.navigate_to_other_pages(category_details,3,navigation,x)
